 ...
 * duration/easing/spring refs, and emits:
 *
 *   css/motion.css                — CSS-var aliases (--dur-, --ease-, --spring-, --tx- families)
 *                                   over the canonical --seed-motion-* tokens (regenerated,
 *                                   replacing the former hand-authored file per ADR 0027)
...
/** Spring tokens (cubic-bezier approximations) → { array, css }, keyed by name. */
const springTokens = Object.fromEntries(
  Object.entries(motionPrims.spring ?? {})
    .filter(([, n]) => n.$value !== undefined)
    .map(([key, n]) => [key, parseEasingValue(n.$value)])
);
...
/** camelCase → kebab-case (easeOutCirc → ease-out-circ). */
...
// ── Builder: css/motion.css (CSS-var alias layer) ─────────────────────────────

/**
 * Emit css/motion.css — ergonomic --dur-, --ease-, --spring-, --tx- aliases over
 * the canonical --seed-motion-* tokens (which tokens.css already defines). This
 * replaces the former hand-authored file: one source of truth (the DTCG seed.motion
 * tree), no drift. Consumer-facing short names are preserved for back-compat.
 */
function buildMotionCss() {
  const lines = [
    '/* AUTO-GENERATED by build-motion-presets.mjs — do not edit directly.          */',
    '/* Source: tokens/core/primitives.json (seed.motion) + seed.durationThemeFade. */',
    '/*                                                                             */',
    '/* Ergonomic motion aliases over the canonical --seed-motion-* tokens.         */',
    '/* Import AFTER tokens.css so the var() references resolve:                     */',
    '/*   @import "nectar-design/tokens.css";                                       */',
    '/*   @import "nectar-design/motion.css";                                       */',
    '',
    ':root {',
    '  /* Duration scale — alias of seed.motion.duration.* (Ant alphabet + grammar) */',
  ];

  for (const name of Object.keys(durationTokens)) {
    lines.push(`  --dur-${name}: var(--seed-motion-duration-${name});`);
  }
  // Theme cross-fade — the canonical engine fade (seed.durationThemeFade = 800ms).
  // Both names kept: --dur-cross (motion vocabulary) + --dur-theme-fade (engine consumers).
  lines.push('  --dur-cross: var(--seed-durationThemeFade);');
  lines.push('  --dur-theme-fade: var(--seed-durationThemeFade);');

  lines.push('', '  /* Easing vocabulary — alias of seed.motion.easing.* (8 Ant curves + grammar) */');
  for (const name of Object.keys(easingTokens)) {
    // Token names already begin with "ease" (easeOut → --ease-out); names that
    // don't (standard) get the ease- prefix so the whole family is namespaced.
    const cssName = name.startsWith('ease') ? toKebab(name) : `ease-${toKebab(name)}`;
    lines.push(`  --${cssName}: var(--seed-motion-easing-${name});`);
  }
  // Legacy alias retained for back-compat (1 consumer): --ease-out-strong == easeOutQuint.
  lines.push('  --ease-out-strong: var(--seed-motion-easing-easeOutQuint);');

  lines.push('', '  /* Spring approximations — alias of seed.motion.spring.* (cubic-bezier) */');
  for (const name of Object.keys(springTokens)) {
    lines.push(`  --spring-${name}: var(--seed-motion-spring-${name});`);
  }

  lines.push('', '  /* Composite transitions — drop in via `transition: var(--tx-*)` */');
  lines.push('  --tx-color: color var(--dur-cross) var(--ease-out),');
  lines.push('              background-color var(--dur-cross) var(--ease-out),');
  lines.push('              border-color var(--dur-cross) var(--ease-out);');
  lines.push('  --tx-ui: transform var(--dur-base) var(--ease-out),');
  lines.push('           opacity var(--dur-base) var(--ease-out);');
  lines.push('  --tx-lift: transform var(--dur-slow) var(--spring-card),');
  lines.push('             box-shadow var(--dur-slow) var(--ease-out);');
  lines.push('  --tx-press: transform var(--dur-press) var(--ease-out-strong);');
  lines.push('}');

  lines.push('');
  lines.push('/* Reduced motion — collapse durations to a near-instant tick so layout');
  lines.push('   transitions still complete but nothing visibly animates. Complements');
  lines.push('   the global safety net in theme.css + the useReducedMotion hook. */');
  lines.push('@media (prefers-reduced-motion: reduce) {');
  lines.push('  :root {');
  for (const name of Object.keys(durationTokens)) {
    lines.push(`    --dur-${name}: 0.01ms;`);
  }
  lines.push('    --dur-cross: 0.01ms;');
  lines.push('    --dur-theme-fade: 0.01ms;');
  lines.push('  }');
  lines.push('}');
...
function emit(filePath, content) {
  ...
  console.log('[build-motion-presets] ✓ %s', rel);
}

emit(join(ROOT, 'css',    'motion.css'),         buildMotionCss());
...
// nosemgrep: format string is a literal; args are internal integer counts, not external input
console.log(
  '[build-motion-presets] done — %d patterns, %d durations, %d easings, %d springs',
  parsedPatterns.length,
  Object.keys(durationTokens).length,
  Object.keys(easingTokens).length,
  Object.keys(springTokens).length
);
